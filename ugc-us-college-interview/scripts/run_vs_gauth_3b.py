#!/usr/bin/env python3
"""
Batch 3 Recovery + Continuation:
  - Cornell: outro was stuck mid-download. Regenerate outro → stitch → Slack
  - Duke, Georgetown, Northwestern, NYU: fresh run

Reuses all logic from run_vs_gauth_3.py (shared imports/functions).
"""
...
# ── Core functions (copied from run_vs_gauth_3.py) ────────────────────────────
...
def build_person_prompt(person: dict, scene: dict) -> str:
    gauth_note = (
        "Vlogger holds phone/mic toward subject and asks the question. Person mentions Gauth first, then clearly favors Professor Curious."
        if person["gauth_first"] else
        "Vlogger holds phone/mic toward subject and asks the question. Person answers directly — Professor Curious."
    )
    return f"""{scene['setting']}
...
Character: {person['type']}, {person['age']}
Emotion/stance: {person['emotion']}
Scene direction: {person['visual']}

{gauth_note}

Vlogger question: "{person['question']}"
Student response: "{person['response']}"

The student speaks slowly and clearly. "Professor Curious" is spoken unmistakably — every syllable.
Night campus. Real. Unscripted-feeling. Not performed.
"""
...
def recover_cornell() -> None:
    """Cornell clips 1-4 already done. Generate outro, stitch all 5, send Slack."""
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  RECOVERY: Cornell — regenerating outro + stitch", flush=True)
    print(f"{'='*60}", flush=True)

    o = CORNELL_OUTRO
    outro_prompt = f"""{o['setting']}
...
"""
    ...
    raw_outro  = raw_dir / "clip_05_outro.mp4"
    logo_outro = logo_dir / "clip_05_outro.mp4"

    print(f"\n  [5/5] Outro (regenerating)", flush=True)
    generate_raw_clip(outro_prompt, "outro", raw_outro)
    embed_logos(raw_outro, logo_outro, gauth_first=False)

    # Collect all 5 logo clips in order
    logo_clip_paths = [
        logo_dir / "clip_01_establishing.mp4",
        logo_dir / "clip_02_p01.mp4",
        logo_dir / "clip_03_p02.mp4",
        logo_dir / "clip_04_p03.mp4",
        logo_dir / "clip_05_outro.mp4",
    ]
    for p in logo_clip_paths:
        if not p.exists():
            raise FileNotFoundError(f"Missing: {p}")

    merged = out_dir / f"{run_id}_merged.mp4"
    print(f"\n  [stitch] Merging 5 logo-embedded clips ...", flush=True)
    stitch_clips(logo_clip_paths, merged)
    send_to_slack(merged, "PC vs Gauth — Cornell")
    print(f"\n  ✓ Cornell DONE: {merged}\n", flush=True)
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"  Batch 3b — Cornell Recovery + Duke/Georgetown/Northwestern/NYU")
    print(f"  Cornell: recover outro → stitch → Slack")
    print(f"  Then: 4 fresh ads")
    print(f"{'#'*60}\n")

    # Step 1: Recover Cornell
    try:
        recover_cornell()
    except Exception as e:
        print(f"  ✗ Cornell FAILED: {e}", flush=True)

    # Step 2: Duke, Georgetown, Northwestern, NYU
    for i, ad in enumerate(ADS, 1):
        print(f"\n[Ad {i}/4] {ad['label']}", flush=True)
        try:
            run_ad(ad)
        except Exception as e:
    ...
    print(f"\n{'#'*60}")
    print(f"  Batch 3b done. Cornell + 4 campuses sent to Slack.")
    print(f"{'#'*60}\n")
